...
class Index(dict):
    ...
    def create_graph(self, modules_list=None):
        ...
        while to_install:
            if sum([add_module(mod) for mod in to_install]) == 0:
    ...
    def get_parents(self, module_name):
        """
        Get all modules (names) which depend on 'module_name'
        """
        return filter(
            lambda module: module_name in self.recursive_deps(module),
            self.keys())

    def recursive_deps(self, module_name):
        """
        get recursive depends (names) for 'module_name'
        """
